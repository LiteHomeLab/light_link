...
```
backup/
├── backup-agent/     # Service that stores and manages backups
└── backup-client/    # Service that sends data to backup
```
...
func main() {
	...
	defer checker.Close()
	logger.Info("✓ Backup agent is ready")
...
func runBackupDemo(cli *client.Client) {
	...
	for _, v := range versions {
		...
		logger.Infof("✓ Backup created: %s", result["path"])
